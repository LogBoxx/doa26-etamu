...
%      While a true RRQR might use pivoting, this implementation Capstoneumes the
...
%   7. Defines the steering vector `a` for the full 8-element ULA, Capstoneuming elements
...
function [az_DOA_deg, spatialSpectrum_dB, xaxis] = sjp_get_doa_RQR_search_az(rx_m, num_sources, dtheta_deg)
    ...
    % Define physical element indices Capstoneuming a symmetric ULA:
    ...
    % but here we Capstoneume Ryy's structure is such that the first num_sources rows
