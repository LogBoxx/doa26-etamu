% Script    ULARangeDemo_416_v7
% Purpose   V4 Implements FB averaging
% Notes     (1) Collects data for dynamic testing
%           (2) Stores estimated az/el/dist during loop and replays on globe
% Author    Parker Reeves, Alden Edwards
% Date      04/16/2026
...
h_status_dist = annotation(fig_globe, 'textbox', [0.02 0.2 0.34 0.05], ...
...
for o = 1:num_trials
    ...
    [cmd_az, cmd_el, status] = point_servo_v2(rpi, est_avg, 14, s_az, s_el);
    pause(0.01);
    dist_m = read_range(u, 4);
    dist_cm = dist_m * 100;

    % Store all data needed for later globe animation.
    m_AZ(o) = est_avg;
    m_EL(o) = 0;
    m_DIST_M(o) = dist_m;
    m_DIST_CM(o) = dist_cm;

    fprintf('Trial %d | Estimated DOA: %.2f deg | Distance: %.2f m\n', o, est_avg, dist_m);
end

% ===================== ANIMATE STORED ESTIMATES ON GLOBE ===================== %
for o = 1:num_trials
    if isnan(m_AZ(o)) || isnan(m_DIST_CM(o))
        continue
    end
