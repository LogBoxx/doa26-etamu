...
opts.lower_el = 20;      % opts.lower_el: minimum allowed elevation after offset (deg)
opts.upper_el = 60;      % opts.upper_el: maximum allowed elevation after offset (deg)
opts.el_offset = 20;      % opts.el_offset: elevation bias added to each input before bounds check
...
% Confidence behavior
opts.alpha_lo = 0.15;      % opts.alpha_lo: blend weight for measurement at lowest confidence
opts.alpha_hi = 0.55;      % opts.alpha_hi: blend weight for measurement at highest confidence
...
opts.strength_min = 500;   % opts.strength_min: strength mapped to confidence 0
opts.strength_max = 12000;  % opts.strength_max: strength mapped to confidence 1
...
opts.serial_keep_latest_bytes = 108; % opts.serial_keep_latest_bytes: bytes to retain when dropping stale backlog
opts.serial_max_read_bytes = 720;   % opts.serial_max_read_bytes: cap for adaptive burst reads
end
