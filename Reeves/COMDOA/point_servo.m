% File: point_servo.m
% Purpose: Tracker wrapper for servo pointing with optional range readout.
%
% Usage:
%   [dist_m, cmd_az, cmd_el, status] = point_servo(r, az_in, el_in, s_az, s_el)
%   [dist_m, cmd_az, cmd_el, status] = point_servo(r, az_in, el_in, s_az, s_el, opts)
%   point_servo('reset')
%
% Inputs are kept identical to get_range_v7 for drop-in compatibility.

function [cmd_az, cmd_el] = point_servo(r, azimuth, elevation, s_az, s_el, opts_in)
...
opts.el_offset = 3;      % opts.el_offset: elevation bias added to each input before bounds check
opts.max_move_az = 5;   % opts.max_move_az: max azimuth step per update (deg)
opts.max_move_el = 2;    % opts.max_move_el: max elevation step per update (deg)
...
% Range read settings
opts.bytes_per_sample = 4;        % opts.bytes_per_sample: sample length for read_range(typecast single)
end
