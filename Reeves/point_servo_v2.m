...
opts.lower_el = 13;      % minimum allowed elevation after offset (deg)
opts.upper_el = 80;      % maximum allowed elevation after offset (deg)
opts.el_offset = 12;      % elevation bias added to each input before bounds check
opts.max_move_az = 5;   % max azimuth step per update (deg)
opts.max_move_el = 3;    % max elevation step per update (deg)

%========Prediction model
opts.window_size = 12;             % number of recent samples kept for regression
opts.min_points_for_fit = 4;      % minimum samples before using regression
opts.predict_horizon_sec = 0.06;  % prediction look-ahead (s)

%========Blending behavior
opts.blend_alpha = 0.80;  % α · measured + (1−α) · predicted = cmd_
