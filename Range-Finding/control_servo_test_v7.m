% File: control_servo_test_v7.m
% Purpose: Hardware demo for get_range_v7 with plotting and status display.
...
% Optional tuning overrides (leave empty struct to use defaults in get_range_v7)
opts = struct(); % opts: override any default field from get_range_v7.
% Example override:
...
fprintf('Starting v7 tracking demo for %d steps...\n', N);
fprintf('Step | az_in el_in | cmd_az cmd_el | dist strength frame_ok\n');
...
for k = 1:N
    ...
    fprintf('%4d | %5.1f %5.1f | %6.1f %6.1f | %6.1f %8.0f %d\n', ...
        k, az_in, el_in, cmd_az, cmd_el, dist_cm, strength, status.lidar.frame_ok);
    pause(dt);
end

% Plot source request, clamped target, servo command, and LiDAR outputs.
figure('Color', 'w', 'Name', 'v7 Source vs Servo + LiDAR');
t = 1:N;
...
grid on;
xlabel('Step');
title('LiDAR Distance and Strength');
legend('Distance', 'Strength', 'Location', 'northwest');
