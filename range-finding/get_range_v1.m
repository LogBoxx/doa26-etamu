...
function [dist_cm, strength] = get_range_v1(r, azimuth,elevation,dev,s_az,s_el)
...
try
  % very small; we only need a few bytes
    ...
    dist_cm  = double(frame(3) + bitshift(frame(4),8));
    strength = double(frame(5) + bitshift(frame(6),8));
    temp_raw = double(frame(7) + bitshift(frame(8),8));
