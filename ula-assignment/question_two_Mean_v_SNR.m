% Script    question_two_Mean_v_SNR
...
theta_true = 30;         % True signal direction (DOA) in degrees
...
for s = 1:length(SNR_dB)
    ...
    mean_est(s) = mean(est_DOAs);               % Compute mean DOA estimate for this SNR
...
%==================== Plotting ==============================
    ...
    grid on;
    ylim([25 35]);                                          % Focus on 25°–35° range
    
    fprintf('SNR = %2d dB --> Mean = %.2f°\n', ...
            SNR_dB(s), mean(est_DOAs));
end

figure;
plot(SNR_dB, mean_est, 'o-', 'LineWidth', 2);               % Plot mean DOA vs. SNR
hold on;
yline(theta_true, 'r--', 'LineWidth', 1.5);                 % True DOA reference line
xlabel('SNR (dB)');
ylabel('Mean Estimated DOA (°)');
title('MUSIC DOA Mean Estimation vs SNR');
grid on;
