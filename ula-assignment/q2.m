...
for s = 1:length(SNR_dB)
    ...
    mean_est(s)  = mean(est_DOAs);
end

%% PLOT PSEUDOSPECTRUM
figure; 
plot(SNR_dB, mean_est,'-o','LineWidth',1.5);
xlabel('SNR (dB)'); ylabel('Mean (degrees)');
ylim([0,90])
title('Mean vs SNR'); grid on;
