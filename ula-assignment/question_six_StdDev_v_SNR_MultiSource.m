% Script    question_six_StdDev_v_SNR_MultiSource
% Purpose  	Calculate the Standard Deviation vs. SNR across 300 MC trials
%               for a Uniform Linear Array with Multiple Sources
% Notes     (1) Number on Antennas: 4
%           (2) Number of Snapshots: 200
%           (3) Source Location: 60 & 75 deg
%           (4) SNR: 0 -> 30dB counting by 5dB
% Author    2026 DoA Team
% Date     	10/6/2025

clc; clear; close all;

M = 8;                     % Number of array elements (increase slightly to resolve two sources)
d = 0.5;                   % Element spacing (in wavelengths)
theta_true = [60 75];      % True signal directions (two sources)
n_snapshots = 200;         % Number of samples per trial
SNR_dB = 0:5:30;           % Range of SNR values to test (dB)
n_trials = 300;            % Number of Monte Carlo trials per SNR
c = 3e8; f = 2.4e9;        % Speed of light and operating frequency
lambda = c/f;              % Wavelength (m)
k = 2*pi/lambda;           % Wave number (rad/m)
delta = d*lambda;          % Element spacing in meters
n_sources = length(theta_true);

std_est = zeros(size(SNR_dB));    % Estimation vector for standard deviation vs. SNR

for s = 1:length(SNR_dB)
    est_DOAs = zeros(n_sources, n_trials);  % Store estimated DOAs for each trial
    
    for mc = 1:n_trials
        A = zeros(M, n_sources);
        for src = 1:n_sources
            A(:, src) = exp(-1j*k*(0:M-1)'*delta*sind(theta_true(src)));  % Steering vector
        end

        s_sig = randn(n_sources, n_snapshots) + 1j*randn(n_sources, n_snapshots);
        X = A*s_sig;
        noise = (randn(M, n_snapshots) + 1j*randn(M, n_snapshots)) / sqrt(2) * 10^(-SNR_dB(s)/20);
        X = X + noise;

        Rxx = (X*X') / n_snapshots;
        [Evec, Eval] = eig(Rxx);
        [~, idx] = sort(diag(Eval), 'descend');
        Evec = Evec(:, idx);
        En = Evec(:, n_sources+1:end);

        theta_scan = -90:0.2:90;
        Pmusic = zeros(size(theta_scan));
        for t = 1:length(theta_scan)
            a_scan = exp(-1j*k*(0:M-1)'*delta*sind(theta_scan(t)));  % Steering vector for scan angle
            Pmusic(t) = 1 / abs(a_scan'*(En*En')*a_scan);            % MUSIC pseudo-spectrum
        end

        [~, locs] = findpeaks(real(Pmusic), theta_scan, 'SortStr','descend','MinPeakDistance',2);
        locs = sort(locs(1:n_sources));        % Take two strongest peaks
        est_DOAs(:, mc) = locs;                % Store both estimated DOAs
    end

    all_estimates = reshape(est_DOAs, 1, []);   % Combine both sources
    std_est(s) = std(all_estimates);            % Total spread metric
    
%==================== Plotting ==============================
    figure;
    stem(1:n_trials, est_DOAs(1,:), 'b', 'filled', 'MarkerSize', 2); hold on;
    stem(1:n_trials, est_DOAs(2,:), 'g', 'filled', 'MarkerSize', 2);
    yline(theta_true(1), 'b--', 'LineWidth', 2);
    yline(theta_true(2), 'g--', 'LineWidth', 2);
    hold off;
    title(sprintf('MUSIC DOA Estimates (Two Sources) at %d dB SNR', SNR_dB(s)));
    xlabel('Monte Carlo Trial');
    ylabel('Estimated DOA (°)');
    legend('Est. Source 1','Est. Source 2','True 60°','True 75°','Location','northwest');
    grid on;
    ylim([50 85]);

    fprintf('SNR = %2d dB --> Mean1 = %.2f°, Mean2 = %.2f°, Std = %.2f°\n', ...
            SNR_dB(s), mean(est_DOAs(1,:)), mean(est_DOAs(2,:)), std_est(s));
end

figure;
plot(SNR_dB, std_est, 'o-', 'LineWidth', 2);
xlabel('SNR (dB)');
ylabel('Standard Deviation of Estimated DOA (°)');
title('MUSIC DOA Estimation Performance vs SNR (Two Sources)');
grid on;



